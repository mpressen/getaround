...
describe 'level4' do
  subject { File.read('level4/data/output.json') }
  let(:expected_output) { File.read('level4/data/expected_output.json') }

  before do
    ENV['LEVEL'] = 'level4/'
    ENV['DISCOUNT'] = 'true'
    ENV['COMMISSION'] = 'true'
    ENV['TRANSACTIONS'] = 'true'
    Main.run
  end

  after do
    File.delete('level4/data/output.json')
  end
